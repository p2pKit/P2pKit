...
| ID | Severity | Finding | Unit/dependencies | Status | Plan/code | Tests | Commit/push | Verification |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| SAMPLE-01 | High data integrity | iOS suppresses sink write failure | SAMPLE-HIGH-01 after XFER-01 | Implemented | iOS sink synchronizes and remembers write failures, surfaces them after terminal state, removes the partial destination, and records a visible error | Generic-simulator Swift app build passes; device sink-failure injection remains | pending focused commit; local, push blocked | Apple/device transfer validation required |
| SAMPLE-02 | High data integrity | iOS same-name destination is non-atomic | SAMPLE-HIGH-01 after XFER-01 | Implemented | iOS sanitized destinations use an atomic `FileManager.createFile` claim with numbered suffixes rather than timestamp-only fallback | Generic-simulator Swift app build passes; concurrent Apple test remains external | pending focused commit; local, push blocked | Apple/device concurrent same-name transfer validation required |
| SAMPLE-03 | High availability/security | Every sample auto-accepts untrusted files | SAMPLE-HIGH-01 after XFER-01 | Implemented | Android, desktop CLI/GUI, and iOS queue incoming offers for explicit accept/reject; acceptance checks 50 MiB quota and free space before opening a file; teardown rejects pending offers | All four sample implementations compile; desktop UI tests 6/6; device consent/timeout evidence remains | pending focused commit; local, push blocked | PS-T04/PS-T07 and physical hostile-sender validation required |
| SAMPLE-04 | High availability | Android retains binary payloads in UI history | SAMPLE-HIGH-01 after XFER-01 | Implemented | Android `RoomMessage` stores timeline text only; binary payloads are converted to a size marker before entering Compose state | Android sample compile; byte-budget/runtime history evidence remains | pending focused commit; local, push blocked | Android instrumentation/soak evidence required |
...
| Source control/post-commit | Pending focused commit at this record update; committed-state hash/checks will be recorded in a tracker-only follow-up. |
