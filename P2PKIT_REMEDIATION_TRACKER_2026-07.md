...
Current finding state: 14 `Planned`, 96 `Implemented`, 29 `Verified`, and 11 `Blocked`. REL-PROV-01 locally implements `BUILD-03/07/09/10/11/12` and `LAN-21`: BuildInfo and XCFramework provenance are declared, reproducible, untracked-input-aware, configuration-cache-safe, exact-commit outputs; the simulator launcher uses one exact UDID, isolated run directories, and serialized project/framework mutation. SAMPLE-LIFE-01 locally implements `SAMPLE-08/09/10/11/16/17/18/19/20/21/25/26/27/28/29/30/31/32/33/36/37/38/39`: sample startup/stop ownership is transactional and retryable; GUI toggles are serialized; CLI parsing, ambiguity, shutdown, and terminal output are deterministic; Android permissions/diagnostics/credential display are corrected; manual endpoints accept scoped IPv6; the KMP demo closes sessions and the kit on failures; and iOS equality/diagnostic/unknown-state handling is bounded. Desktop sample tests pass 16/16, Android compile/lint pass, the KMP JVM tests pass 2/2, and the provenance-fresh generated iOS app builds for the generic simulator; physical UI/lifecycle/permission and hostile-network evidence remains external. SAMPLE-HIGH-01 locally implements `SAMPLE-01/02/03/04`: Android, desktop CLI/GUI, and iOS sample receivers queue offers for explicit accept/reject consent, enforce a 50 MiB quota and free-space check before opening destinations, and reject pending offers during teardown; iOS claims sanitized same-name destinations atomically and surfaces sink write failures with partial-file cleanup; Android timeline rows retain only bounded text representations instead of binary payloads. PROV-D-01 locally implements `PROV-D01..05` and `PS-T03`; PROV-A-01 and PROV-A-HIGH-01 locally implement `PROV-A01..11` except the real-callback-only `PROV-A12` gap; PATH-PERM-01 locally implements `CORE-22/25/26` and `CORE-T12`; LAN-NET-01, LAN-NET-SCHEMA-01, LAN-APPLE-03, LAN-JMDNS-02, and LAN-CONN-01 locally implement their recorded LAN findings while retaining explicit physical/device and hostile-network evidence. CORE-LIFE-02 locally implements `CORE-12/16` and the discovery half of `CORE-11`; data-start rollback and provisioning disposal are isolated at `DATA-TRANSPORT-LIFECYCLE-API-01` and `PROVISIONING-CLOSE-API-01`. PEER-CTRL-01 locally implements `CORE-02/04/05/14`; `CORE-15/17/27/28` remain at documented public-API/model decisions. XFER-01 verified `PROTO-04` and `FILE-01/02/03/07/08/09/10/12/14`; `FILE-15` is implemented pending real Android-provider instrumentation, while `FILE-05/06/11` remain at documented API decisions. SEC-01 remains implemented but externally gated by cryptographic audit, physical interoperability, hostile-network validation, and the final green repository gate.
...
- CORE-11 data-start rollback requires a restartable transport-stop contract; terminal `close()` cannot be used without destroying the documented retry path.
- CORE-24 requires an explicit public provisioning-manager disposal contract; parent-job attachment alone cannot guarantee third-party resource ownership.
...
| CORE-11 | Medium | Partial startup/advertising/discovery is not rolled back | CORE-LIFE-02 + DATA-TRANSPORT-LIFECYCLE-API-01 | Blocked | Advertising/discovery attempts roll back in reverse on failure/cancellation; data startup needs restartable stop semantics | CORE-T09 | `60aecf6` partial; local, push blocked | Discovery rollback JVM/iOS green; exact owner decision recorded below |
...
| CORE-17 | Medium | Public values expose mutable backing storage | IMMUTABLE-MODEL-API-01 | Blocked | Binary and registry ownership snapshots implemented; public data-class collection getters still require a model/API change for cast-proof immutability | Mutation/ownership tests | `ee2cae5` partial; push blocked | Owner compatibility decision recorded below |
...
| CORE-24 | Medium API gap | NetworkProvisioningManager has no close contract | PROVISIONING-CLOSE-API-01 | Blocked | Requires an explicit idempotent suspending public close contract and implementer migration | Close contract/ABI tests | — | Owner API decision recorded below |
...
| CORE-T09 | Partial multi-transport startup rollback | CORE-11 | Blocked | Advertising failure and discovery cancellation roll back every attempted transport; data-start conjunct awaits restartable stop contract | `60aecf6` partial | Exact owner decision recorded below |
...
| Gap ID | Required coverage | Linked findings | Status | Test files/evidence | Commit/push | Verification |
| --- | --- | --- | --- | --- | --- | --- |
| LAN-T01 | Android instrumentation for multicast locks, callback ordering, network rotation, permissions, unregister failure, process binding, and IPv6 | LAN-10, LAN-13, LAN-19 | In Progress | Common lifecycle fakes cover retained callback/lock ownership and Android production compiles; real platform behavior remains | `83db1ab` partial; push blocked | Physical/emulated Android required |
...
| LAN-T07 | Apple browser/listener recovery, start-close race, parent cancellation, inbound flood/drain, cache-prune race, same-type path rotation, and AWDL | LAN-03, LAN-04, LAN-11, LAN-12, LAN-13, LAN-17 | In Progress | Browser/listener recovery, start cancellation, cancelled close, bounded inbound drain, outbound cancellation, cache-prune race, same-interface address-change rebind decision, and peer-to-peer parameter enablement implemented; physical path/AWDL interop remains | `b21ed84`, `703182a`, `21bc2fc`; push blocked | Complete Apple LAN suite 60 tests/0 failures/1 manual skip; native seam passes; physical path/AWDL evidence remains |
| LAN-T08 | Oversized TXT, unsupported pv, control characters, duplicate-PID spoof, off-subnet hosts, alternate address candidates, and connection flood | LAN-04, LAN-15, LAN-16, LAN-18, LAN-22 | In Progress | Connection flood in `b21ed84`; TXT size/version/control/service-identity and diagnostic conjuncts in `2895ff1`/`09af6b5`; off-subnet filtering, eight-candidate preservation, and failed-first fallback in `21bc2fc`; exact TXT-less service-admission ownership in `639c508` | `b21ed84`, `2895ff1`, `09af6b5`, `21bc2fc`, `639c508`; push blocked | Deterministic admission/removal 5/5 and complete LAN JVM 83/83 pass; hostile-network/device evidence remains required |
...
| Gap ID | Required coverage | Linked findings | Status | Test files/evidence | Commit/push | Verification |
| --- | --- | --- | --- | --- | --- | --- |
| PS-T01 | Real Android provisioning callback/cancellation and permission behavior | PROV-A01, PROV-A02, PROV-A03, PROV-A04, PROV-A05, PROV-A06, PROV-A07, PROV-A09, PROV-A10, PROV-A12 | In Progress | Host JVM seams now cover failed-result reservation cleanup, parent cancellation, late callback ownership, input boundaries, LinkProperties projection, and bind/permission decision paths; physical callback ordering, OEM return values, merged-manifest/logcat, and hardware matrix remain | `21dad61` plus `5bc31ce`; local, push blocked | Android instrumentation required |
| PS-T02 | Process-wide binding ownership across managers | PROV-A05, PROV-A08, PROV-A12 | In Progress | Deterministic host `ProcessBindingArbiter` ownership test added; two-manager real process-binding/unbind proof remains | `21dad61`; local, push blocked | Android instrumentation required |
...
| PS-T04 | Android sample unit/instrumentation tests | SAMPLE-03, SAMPLE-04, SAMPLE-05, SAMPLE-06, SAMPLE-07, SAMPLE-08, SAMPLE-10, SAMPLE-14, SAMPLE-15, SAMPLE-16, SAMPLE-17, SAMPLE-25, SAMPLE-26, SAMPLE-27, SAMPLE-28, SAMPLE-34, SAMPLE-36, SAMPLE-39, BUILD-08, BUILD-15 | In Progress | Consent/storage/cleanup/history and lifecycle/permission/toggle implementations compile; `compileDebugKotlin` and `lintDebug` pass, while runtime UI, permission, cancellation, collision, rapid-toggle, partial-start, and `onCleared` instrumentation remains | `c4b31a1`, `9102b78`, `b90b8bd`; local, push blocked | Android instrumentation/physical hardware required |
| PS-T05 | CLI option parsing, target ambiguity, shutdown finally, and terminal collector cleanup | SAMPLE-07, SAMPLE-11, SAMPLE-18, SAMPLE-19, SAMPLE-37, SAMPLE-38 | In Progress | Parser regressions pass 5/5 and CLI compiles with deterministic ambiguity, close reporting, job ordering, and finally cleanup; injected runtime close/REPL/shutdown failure and hostile-terminal evidence remains | `9102b78`, `b90b8bd`; local, push blocked | CLI integration/runtime evidence required |
| PS-T06 | Desktop GUI create/stop races, accept cancellation, and byte-budgeted history | SAMPLE-06, SAMPLE-08, SAMPLE-09, SAMPLE-10, SAMPLE-22, SAMPLE-24, SAMPLE-33, SAMPLE-34 | In Progress | Desktop UI path/terminal tests pass 11/11 and lifecycle ownership compiles; injected create/stop failure, rapid toggle, window disposal, accept cancellation, and byte-budget soak evidence remains | `9102b78`, `b90b8bd`; local, push blocked | Desktop GUI integration/runtime evidence required |
| PS-T07 | Swift sink failure, atomic filename collision, partial-file cleanup, and unstructured collector cancellation | SAMPLE-01, SAMPLE-02, SAMPLE-05, SAMPLE-12, SAMPLE-13, SAMPLE-23, SAMPLE-24 | In Progress | Swift transfer cleanup now removes non-completed partial files, task handles are lifecycle-owned, iOS stop orders SDK quiescence before watcher cancellation, and histories are bounded; device failure/collision/soak evidence remains | `c4b31a1` plus SAMPLE-XFER local code; push blocked | Apple unit-test target/device required |
| PS-T08 | Dot-segment names and scoped IPv6 on every sample | SAMPLE-15, SAMPLE-20, SAMPLE-34 | In Progress | Filename/peer-name claims reject path and dot segments; endpoint tests cover DNS, IPv4, bracketed/unbracketed IPv6, and `%` scope IDs | `9102b78`, `b90b8bd`; local, push blocked | Every-sample physical scoped-IPv6 and hostile-name evidence required |
| PS-T09 | KMP Android runtime consumer and iOS consumer target | SAMPLE-21, BUILD-01 | In Progress | BUILD-01 consumers compile and KMP demo cleanup is exercised by 2/2 JVM loopback/callsite tests | `8f15d75`, `b90b8bd`; local, push blocked | Physical Android/iOS consumer runtime remains |
...
| Remaining evidence/blockers | JVM cannot portably prove a default route or physical Wi-Fi association; real OS route/interface behavior remains outside host tests. `PROVISIONING-CLOSE-API-01` remains the final disposal-contract blocker; push remains blocked by `SCM-PUSH-01`. |
...
`DATA-TRANSPORT-LIFECYCLE-API-01` blocker (`CORE-11` data-start conjunct, `CORE-T09`): each shipped data transport treats `close()` as terminal and rejects a later `start()`. Closing transport A when transport B fails would therefore make the documented retry path fail permanently; not closing A leaks a partial startup. Work attempted: confirmed the contract and shipped JVM/Android/Apple behavior, implemented every independent discovery rollback, and stopped before inventing terminal-vs-restart semantics. Recommended option: add an idempotent restartable `suspend stop()` contract to `DataTransport`, implement it on all shipped transports, reserve `close()` for terminal disposal, and roll back attempted starts in reverse. Alternatives: rebuild the complete transport/registry/session graph after a failure (larger ownership/race surface), or make any partial start failure terminal for the kit (breaking the documented retry contract). Exact reply: `Approve DATA-TRANSPORT-LIFECYCLE-API-01 restartable stop`. This unblocks the remaining `CORE-11/CORE-T09` conjunct and later restart/rebind lifecycle work.

`PROVISIONING-CLOSE-API-01` blocker (`CORE-24` and the terminal-disposal dependency of provisioning batches): `NetworkProvisioningManager` has no disposal method, and an external implementation may ignore or mis-parent the advisory `parentJob`. Work attempted: traced construction and terminal ownership; no source behavior was guessed. Recommended option: add a required idempotent suspending `close()` method, implement bounded cleanup on every built-in manager, invoke it before kit-scope cancellation, and add ABI/external-implementer migration tests. Alternatives: a default no-op method (source-compatible but cannot guarantee cleanup), or an optional closeable sub-interface (leaves some managers undisposable). Exact reply: `Approve PROVISIONING-CLOSE-API-01 required close`. This unblocks `CORE-24` and the final lifecycle integration of Android/desktop provisioning; independent provisioning internals may proceed meanwhile.
