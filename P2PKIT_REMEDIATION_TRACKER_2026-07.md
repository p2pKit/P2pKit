...
Current finding state: 44 `Planned`, 67 `Implemented`, 29 `Verified`, and 11 `Blocked`. SAMPLE-HIGH-01 now locally implements `SAMPLE-01/02/03/04`: Android, desktop CLI/GUI, and iOS sample receivers queue offers for explicit accept/reject consent, enforce a 50 MiB quota and free-space check before opening destinations, and reject pending offers during teardown; iOS claims sanitized same-name destinations atomically and surfaces sink write failures with partial-file cleanup; Android timeline rows retain only bounded text representations instead of binary payloads. Android and both desktop samples compile, the desktop UI tests pass 6/6, and the generated iOS app builds for the generic simulator; physical/device transfer proof remains external. PROV-D-01 now locally implements `PROV-D01..05` and `PS-T03`: desktop polling rejects non-positive intervals before starting, filters down/virtual/link-local candidates and ranks private LAN addresses, propagates fatal scanner errors while handling socket failures narrowly, proves null/non-null manual connection behavior with an injected scanner, and removes process-global `user.home` mutation from the loopback test. PROV-A-01 now locally implements `PROV-A01/A04/A05/A06/A07/A08/A09/A10/A11`: Android start-result cleanup closes a reservation before returning a typed failure; hotspot/join callback ownership is serialized across cancellation, unavailable, reconnect, and close races; process binding is checked and arbitrated; joined snapshots use `LinkProperties`; normal manifest omissions are diagnosed; and Wi-Fi input boundaries are validated before platform builders. Real Android callback/process-binding/permission and hardware evidence remains open in `PS-T01/PS-T02`, and `PROV-A12` remains a real-callback test gap. PROV-A-HIGH-01 now locally implements `PROV-A03` and the shared terminal-gate portion of `PROV-A02`: Android close/parent cancellation atomically claims owned hotspot/join handles, and callbacks completing after close release their late handles and return one deterministic failure; real Android callback/process-binding evidence remains open. PATH-PERM-01 now locally implements `CORE-22/25/26` and `CORE-T12` with generation-gated Android/iOS observer ownership, context-backed Android default wiring, and the complete four-permission manifest diagnostic; physical Android/iOS callback and manifest/logcat evidence remains open. LAN-NET-01 now locally implements the liveness, native-removal, route-rotation, AWDL-parameter, subnet-affinity, and bounded alternate-candidate portions of `LAN-13/14/16/17/18`; physical/device and hostile-network evidence remains open. LAN-NET-SCHEMA-01 locally implements `LAN-15/22` and the bounded-schema/service-ownership portion of `LAN-16/LAN-T08`. LAN-APPLE-03 locally implements `LAN-03/11/12` and `LAN-T10`; `LAN-T07` remains in progress only for physical same-interface rotation/AWDL evidence. LAN-JMDNS-02 locally implements `LAN-01/02/10/25/26` and gaps `LAN-T02/T03`; real Android callback instrumentation remains in LAN-T01. LAN-CONN-01 locally implements `LAN-04/05/06/07/08/09/23/24`. CORE-LIFE-02 locally implements `CORE-12/16` and the discovery half of `CORE-11`; data-start rollback and provisioning disposal are isolated at `DATA-TRANSPORT-LIFECYCLE-API-01` and `PROVISIONING-CLOSE-API-01`. PEER-CTRL-01 locally implements `CORE-02/04/05/14`; `CORE-15/17/27/28` remain at documented public-API/model decisions. XFER-01 verified `PROTO-04` and `FILE-01/02/03/07/08/09/10/12/14`; `FILE-15` is implemented pending real Android-provider instrumentation, while `FILE-05/06/11` remain at documented API decisions. SEC-01 remains implemented but externally gated by cryptographic audit, physical interoperability, hostile-network validation, and the final green repository gate.
...
| PS-T04 | Android sample unit/instrumentation tests | SAMPLE-03, SAMPLE-04, SAMPLE-05, SAMPLE-06, SAMPLE-07, SAMPLE-08, SAMPLE-10, SAMPLE-14, SAMPLE-15, SAMPLE-16, SAMPLE-17, SAMPLE-25, SAMPLE-26, SAMPLE-27, SAMPLE-28, SAMPLE-34, SAMPLE-36, SAMPLE-39, BUILD-08, BUILD-15 | In Progress | Android consent/quota/free-space, terminal cleanup, atomic claim, and byte-history implementations compile; runtime UI, storage, cancellation, collision, and byte-budget instrumentation remains | `c4b31a1` plus SAMPLE-XFER local code; push blocked | Android instrumentation required |
| PS-T05 | CLI option parsing, target ambiguity, shutdown finally, and terminal collector cleanup | SAMPLE-07, SAMPLE-11, SAMPLE-18, SAMPLE-19, SAMPLE-37, SAMPLE-38 | In Progress | CLI terminal transfer collector and destination-claim failure paths compile; option parsing, shutdown, and runtime terminal evidence remain | SAMPLE-XFER local code; push blocked | CLI integration/runtime evidence required |
| PS-T06 | Desktop GUI create/stop races, accept cancellation, and byte-budgeted history | SAMPLE-06, SAMPLE-08, SAMPLE-09, SAMPLE-10, SAMPLE-22, SAMPLE-24, SAMPLE-33, SAMPLE-34 | In Progress | Desktop UI sink cleanup, deterministic claim failure, atomic/path tests, and byte-bounded room history compile; GUI lifecycle/cancellation/soak evidence remains | SAMPLE-XFER local code; push blocked | Desktop GUI integration/runtime evidence required |
...
| PS-T08 | Dot-segment names and scoped IPv6 on every sample | SAMPLE-15, SAMPLE-20, SAMPLE-34 | In Progress | Android/desktop/iOS filename and peer-name claims reject path/dot segments; scoped IPv6 coverage remains with SAMPLE-LIFE-01 | SAMPLE-XFER local code; push blocked | Hostile-name/device and IPv6 evidence required |
