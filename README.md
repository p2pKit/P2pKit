...
- **v0.5 residual edge case — iPhone Wi-Fi flap with cellular enabled.** When iPhone cellular data is enabled and Wi-Fi is toggled, iOS's `Network.framework` may briefly select the cellular interface during the Wi-Fi gap and rotate the listener through an intermediate cellular-only port that Android cannot reach over LAN; the eventual post-Wi-Fi-back announce can arrive after Android's reconnect budget has exhausted. This is an iOS path-selection behaviour during interface handover, not a cache-staleness issue, and reproduces only with cellular enabled during the flap. **Workaround**: disable iPhone cellular data for LAN-only reconnect, or initiate `kit.connect(peer)` from the iPhone side after the flap (the iPhone's outgoing path works because Android's listener port does not move on iPhone Wi-Fi flap).
