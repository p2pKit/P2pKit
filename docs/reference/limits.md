...
| `FileTransferConfig.offerTimeoutMillis` | `30,000` ms (30 s) | Positive offer-decision/operation-wait budget; unanswered receiver offers auto-reject. Also accepted-transfer idle and commit/ack wait; see derived deadlines below. | [FileTransferConfig.kt:35](../../library/p2p-core/src/commonMain/kotlin/dev/p2pkit/core/transfer/FileTransferConfig.kt#L35) |
| `FileTransferConfig.maxConcurrentIncomingBytes` | `8,589,934,592` bytes (8 GiB) | Positive sum of declared sizes of active incoming offers/transfers per session, reserved before opening a sink. Further offers are rejected; terminal transfers release accounting. | [FileTransferConfig.kt:46](../../library/p2p-core/src/commonMain/kotlin/dev/p2pkit/core/transfer/FileTransferConfig.kt#L46) |
...
[JVM](../../library/p2p-core/src/jvmMain/kotlin/dev/p2pkit/core/transfer/FileTransferDestinationJvm.kt#L40-L55)
and [Android](../../library/p2p-core/src/androidMain/kotlin/dev/p2pkit/core/transfer/FileTransferDestinationAndroid.kt#L35-L53)
factories; application-provided destinations own their storage policy.

Reconnect is **disabled by default**. [`ReconnectPolicy.Enabled`](../../library/p2p-core/src/commonMain/kotlin/dev/p2pkit/core/Config.kt#L61-L66)
...
[receiver enters finalization](../../library/p2p-core/src/commonMain/kotlin/dev/p2pkit/core/internal/FileTransferDispatcher.kt#L2021-L2026)
or the [sender enters COMMIT_WAIT](../../library/p2p-core/src/commonMain/kotlin/dev/p2pkit/core/internal/FileTransferDispatcher.kt#L1511-L1516).
...
[FileTransferConfig](../../library/p2p-core/src/commonMain/kotlin/dev/p2pkit/core/transfer/FileTransferConfig.kt#L96-L129)
...
| Policy or setting | Current value | Scope and observable consequence | Source |
| --- | --- | --- | --- |
| `MAX_CONCURRENT_PRE_HANDSHAKE_SETUPS` | `16` setups | Per kit, inbound pre-handshake work only. At capacity, new inbound connections are warned about and closed; outgoing setups are exempt. | [SessionManager.kt:1833](../../library/p2p-core/src/commonMain/kotlin/dev/p2pkit/core/internal/SessionManager.kt#L1833) |
| `MAX_TOTAL_ACTIVE_SESSIONS` | `64` active sessions | Only active entries count. Net-new inbound registration at capacity is refused, including replacement of a terminal entry. Outgoing registrations and no-growth simultaneous-open arbitration are exempt; not a hard total-session ceiling. Warn and close; no required typed local application error. | [SessionManager.kt:1891](../../library/p2p-core/src/commonMain/kotlin/dev/p2pkit/core/internal/SessionManager.kt#L1891) |
...
| `MAX_BUFFERED_INBOUND_CONNECTIONS` | `16` connections | Per LAN transport, accepted connections awaiting collection on JVM, Android and Apple. Distinct from active setups/sessions; overflow closes the newest connection. | [Lan.kt:188](../../library/p2p-transport-lan/src/commonMain/kotlin/dev/p2pkit/transport/lan/Lan.kt#L188) |
| `MAX_QUEUED_APPLICATION_MESSAGES` | `64` messages | Per session, includes the message currently being emitted to incoming. Further admission fails the session and logs the receive-backlog reason. | [P2pSessionImpl.kt:1791](../../library/p2p-core/src/commonMain/kotlin/dev/p2pkit/core/internal/P2pSessionImpl.kt#L1791) |
| `MAX_QUEUED_APPLICATION_BYTES` | `8,388,608` bytes (8 MiB) | Same backlog: approximate retention-policy charge, including the in-flight message. Exceeding it fails the session; not wire bytes or a whole-session/process heap cap. | [P2pSessionImpl.kt:1794](../../library/p2p-core/src/commonMain/kotlin/dev/p2pkit/core/internal/P2pSessionImpl.kt#L1794) |
...
payload. The [exact flight definitions](../../library/p2p-core/src/commonMain/kotlin/dev/p2pkit/core/internal/security/noise/SecureProtocolV2Wire.kt#L143-L149)
...
| Policy or setting | Current value | Scope and observable consequence | Source |
| --- | --- | --- | --- |
| `MAX_DISCOVERED_PEERS` | `1,024` peers | Distinct peers with discovery contributions per kit; manual-only entries are not counted. New discovered identities are ignored at capacity; existing updates/removals still work. | [PeerRegistry.kt:630](../../library/p2p-core/src/commonMain/kotlin/dev/p2pkit/core/internal/PeerRegistry.kt#L630) |
...
| `DEFAULT_STALE_TIMEOUT_MS` | `15,000` ms (15 s) | Unrefreshed discovery contributions older than this are removed unless transport-managed. LAN uses transport-managed lifetime; manual entries are exempt. Not a universal LAN peer TTL. | [PeerRegistry.kt:628](../../library/p2p-core/src/commonMain/kotlin/dev/p2pkit/core/internal/PeerRegistry.kt#L628) |
| `DEFAULT_EVICTION_POLL_MS` | `1,000` ms (1 s) | Core stale-eviction polling interval. Eligible stale contributions disappear on a later poll, not at an exact wall-clock instant. | [PeerRegistry.kt:629](../../library/p2p-core/src/commonMain/kotlin/dev/p2pkit/core/internal/PeerRegistry.kt#L629) |
| `MAX_DISCOVERY_HINTS` | `32` hints | Per individual discovery claim/contribution. More routing hints reject that claim; merging distinct hints from multiple contributions can exceed this number for one peer. | [PeerRegistry.kt:637](../../library/p2p-core/src/commonMain/kotlin/dev/p2pkit/core/internal/PeerRegistry.kt#L637) |
| `MAX_DISCOVERY_METADATA_ENTRIES` | `16` entries/hint | Per discovery transport hint; a claim exceeding this metadata count is rejected. | [PeerRegistry.kt:638](../../library/p2p-core/src/commonMain/kotlin/dev/p2pkit/core/internal/PeerRegistry.kt#L638) |
| `MAX_DISCOVERY_METADATA_KEY_CHARS` | `64` characters | Discovery metadata key length; the claim is rejected if exceeded (and its UTF-8 bound also applies). | [PeerRegistry.kt:639](../../library/p2p-core/src/commonMain/kotlin/dev/p2pkit/core/internal/PeerRegistry.kt#L639) |
| `MAX_DISCOVERY_METADATA_KEY_UTF8_BYTES` | `256` bytes | Discovery metadata key UTF-8 ceiling; exceeding it rejects the claim. | [PeerRegistry.kt:640](../../library/p2p-core/src/commonMain/kotlin/dev/p2pkit/core/internal/PeerRegistry.kt#L640) |
| `MAX_DISCOVERY_METADATA_VALUE_CHARS` | `256` characters | Discovery metadata value length; exceeding it rejects the claim. | [PeerRegistry.kt:642](../../library/p2p-core/src/commonMain/kotlin/dev/p2pkit/core/internal/PeerRegistry.kt#L642) |
| `MAX_DISCOVERY_METADATA_VALUE_UTF8_BYTES` | `1,024` bytes | Discovery metadata value UTF-8 ceiling; exceeding it rejects the claim. | [PeerRegistry.kt:643](../../library/p2p-core/src/commonMain/kotlin/dev/p2pkit/core/internal/PeerRegistry.kt#L643) |
| `MAX_DISCOVERY_HOST_CHARS` | `253` characters | Discovery hint host length; excess rejects the claim. Blank, control-bearing or whitespace-containing hosts are also invalid. | [PeerRegistry.kt:635](../../library/p2p-core/src/commonMain/kotlin/dev/p2pkit/core/internal/PeerRegistry.kt#L635) |
| `MAX_DISCOVERY_HOST_UTF8_BYTES` | `1,012` bytes | Discovery hint host UTF-8 ceiling; exceeding it rejects the claim. | [PeerRegistry.kt:636](../../library/p2p-core/src/commonMain/kotlin/dev/p2pkit/core/internal/PeerRegistry.kt#L636) |
| `MAX_MANUAL_HOST_CHARS` | `253` characters | Normalized manual host length; invalid local registration throws rather than creating a peer. | [PeerRegistry.kt:634](../../library/p2p-core/src/commonMain/kotlin/dev/p2pkit/core/internal/PeerRegistry.kt#L634) |
...
| `WRITE_TIMEOUT_MILLIS` | `30,000` ms (30 s) | JVM raw socket-write watchdog. Expiry closes the socket and fails the write; excludes time waiting for the write mutex. | [JvmRawConnection.kt:284](../../library/p2p-transport-lan/src/jvmMain/kotlin/dev/p2pkit/transport/lan/JvmRawConnection.kt#L284) |
| `WRITE_TIMEOUT_MILLIS` | `30,000` ms (30 s) | Android raw socket-write watchdog, with the same close/fail behavior and mutex-wait exclusion. | [AndroidRawConnection.kt:280](../../library/p2p-transport-lan/src/androidMain/kotlin/dev/p2pkit/transport/lan/AndroidRawConnection.kt#L280) |
...
| Policy or setting | Current value | Scope and observable consequence | Source |
| --- | --- | --- | --- |
| `HANDSHAKE_CLEANUP_TIMEOUT_MS` | `2,000` ms (2 s) | Per resource/join during incomplete-handshake rollback; expiry reports incomplete cleanup. Also the best-effort CLOSE opportunity: its expiry proceeds to transport teardown without necessarily creating a cleanup issue. | [SessionManager.kt:1836](../../library/p2p-core/src/commonMain/kotlin/dev/p2pkit/core/internal/SessionManager.kt#L1836) |
| `SESSION_COMMIT_CLEANUP_TIMEOUT_MS` | `2,000` ms (2 s) | Per raw connection during failed session-publication rollback. Expiry reports incomplete cleanup. | [SessionManager.kt:1839](../../library/p2p-core/src/commonMain/kotlin/dev/p2pkit/core/internal/SessionManager.kt#L1839) |
| `DEFAULT_DISCOVERY_REFRESH_TIMEOUT_MS` | `6,000` ms (6 s) | One reconnect discovery-refresh callback. Timeout is logged and does not itself exhaust the reconnect policy. | [SessionManager.kt:1842](../../library/p2p-core/src/commonMain/kotlin/dev/p2pkit/core/internal/SessionManager.kt#L1842) |
| `SESSION_CLOSE_TIMEOUT_MS` | `10,000` ms (10 s) | Manager wait for each session/setup during background or terminal cleanup; separate from the session's 8 s controlled-wait budget. Expiry is a cleanup issue, not a clean-close guarantee. | [SessionManager.kt:1845](../../library/p2p-core/src/commonMain/kotlin/dev/p2pkit/core/internal/SessionManager.kt#L1845) |
| `SESSION_CLOSE_BUDGET_MS` | `8,000` ms (8 s) | Aggregate elapsed allowance across terminal session controlled waits and followers; not a preemptive API or native-release guarantee. | [SessionCleanupBudget.kt:22](../../library/p2p-core/src/commonMain/kotlin/dev/p2pkit/core/internal/SessionCleanupBudget.kt#L22), [P2pSessionImpl.kt:1785](../../library/p2p-core/src/commonMain/kotlin/dev/p2pkit/core/internal/P2pSessionImpl.kt#L1785) |
| `CLOSE_FRAME_TIMEOUT_MS` | `2,000` ms (2 s) | Best-effort CLOSE wait, further capped by the aggregate remainder. Retirement of that exact epoch also ends the wait. Expiry proceeds to teardown; delivery is not guaranteed. | [P2pSessionImpl.kt:1777](../../library/p2p-core/src/commonMain/kotlin/dev/p2pkit/core/internal/P2pSessionImpl.kt#L1777) |
| `SESSION_RESOURCE_CLOSE_TIMEOUT_MS` | `2,000` ms (2 s) | Each terminal resource observation (also default application-delivery join), further capped by the aggregate remainder. Expiry reports pending cleanup, not a cancelled owner. | [P2pSessionImpl.kt:1788](../../library/p2p-core/src/commonMain/kotlin/dev/p2pkit/core/internal/P2pSessionImpl.kt#L1788) |
| `SESSION_RUNTIME_CLOSE_TIMEOUT_MS` | `2,000` ms (2 s) | Final runtime join, further capped by the aggregate remainder. Cancellation is still requested at zero allowance; an incomplete join reports pending runtime termination. | [P2pSessionImpl.kt:1789](../../library/p2p-core/src/commonMain/kotlin/dev/p2pkit/core/internal/P2pSessionImpl.kt#L1789) |
| `STOP_START_MUTEX_TIMEOUT_MS` | `5,000` ms (5 s) | Kit stop waiting for an active startup lock. Expiry warns, records a cleanup issue and continues terminal teardown without that lock. | [P2pKitImpl.kt:1548](../../library/p2p-core/src/commonMain/kotlin/dev/p2pkit/core/internal/P2pKitImpl.kt#L1548) |
| `OBSERVER_CLOSE_TIMEOUT_MS` | `5,000` ms (5 s) | Kit path-observer close wait. Expiry records incomplete observer cleanup. | [P2pKitImpl.kt:1556](../../library/p2p-core/src/commonMain/kotlin/dev/p2pkit/core/internal/P2pKitImpl.kt#L1556) |
| `INTERNAL_JOB_CLOSE_TIMEOUT_MS` | `5,000` ms (5 s) | Kit internal-job cancellation/join wait. Expiry records failure and retains the destructive-identity-reset lease fail-closed. | [P2pKitImpl.kt:1559](../../library/p2p-core/src/commonMain/kotlin/dev/p2pkit/core/internal/P2pKitImpl.kt#L1559) |
| `STALE_OPERATION_CLEANUP_TIMEOUT_MS` | `2,000` ms (2 s) | Each resource produced by a stale lifecycle operation. Expiry is reported; late work is not declared cleaned. | [P2pKitImpl.kt:1562](../../library/p2p-core/src/commonMain/kotlin/dev/p2pkit/core/internal/P2pKitImpl.kt#L1562) |
| `RESOURCE_CLOSE_TIMEOUT_MS` | `6,000` ms (6 s) | Each kit-owned transport/provisioning close attempt. Expiry reports incomplete cleanup instead of claiming success. | [P2pKitImpl.kt:1573](../../library/p2p-core/src/commonMain/kotlin/dev/p2pkit/core/internal/P2pKitImpl.kt#L1573) |
| `DEFAULT_FEATURE_OPERATION_SETTLE_TIMEOUT_MS` | `6,000` ms (6 s) | Explicit advertising/discovery stop waiting for the concurrent start owner. Expiry fails the operation with retained cleanup context. | [P2pKitImpl.kt:1615](../../library/p2p-core/src/commonMain/kotlin/dev/p2pkit/core/internal/P2pKitImpl.kt#L1615) |
