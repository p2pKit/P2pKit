...
| [#23 — iOS stale Bonjour endpoint after peer restart](https://github.com/p2pKit/P2pKit/issues/23) | Commits `d6efe08` and `7af3a4b` give cached endpoints immutable browser-generation leases, replace them on each accepted update, retire the browser callback owner before the final registry clear, and conditionally remove only the failed lease so a concurrent fresh result survives. A connected dial cannot cross the final stop/rebind ownership handoff. | Prove peer-restart timing on devices. The first stale attempt can still consume the evidence-dependent 10-second Apple connect ceiling; the fix prevents that failed lease from remaining dialable or deleting a newer endpoint. | **High, external evidence only.** Apple devices/AWDL. Execute `LAN-T07` restart/path cases and retain both-peer endpoint-generation/timing evidence. |
...
| [#27 — iOS hotspot rebind/foreground storm](https://github.com/p2pKit/P2pKit/issues/27) | Commit `d6efe08` includes `nw_interface_type_other` in the path fingerprint and coalesces WillEnterForeground/DidBecomeActive into one inactive episode. Commit `7af3a4b` restores discovery from current host intent rather than a stale pre-rebind flag and cancels delayed Bonjour descriptor nudges when their listener retires. | Personal Hotspot path classification and notification timing still need physical-device proof across iOS versions. | **Medium, external evidence only.** Apple device/Personal Hotspot. Run `LAN-T07` repeated enable/foreground cases and prove at most one completed rebind per inactive episode. |
| [#28 — Android address order may select VPN](https://github.com/p2pKit/P2pKit/issues/28) | Commit `1ee7ee1` centralizes discovery/manual-dial selection on one `Network`'s `LinkProperties`, prioritizes explicit safe address classes instead of raw list order, rejects VPN/cellular overlays even when they also report an underlying LAN transport, and shares one immutable route snapshot with TCP. Android-host tests pin order independence and overlay rejection. | Validate VPN plus Wi-Fi/hotspot combinations on real OEM devices and prove both discovery and TCP stay on the chosen LAN. | **High, external evidence only.** Execute `LAN-T01` with PCAP/dumpsys evidence. |
| [#29 — Android rebind cancellation race](https://github.com/p2pKit/P2pKit/issues/29) | `JmdnsLifecycleCoordinator` serializes transitions and commit `1ee7ee1` makes an already-started close/recreate ownership transaction non-cancellable. A superseding debounce may cancel only waiting work; deterministic tests park native close/create, supersede or stop concurrently, and prove ordered replacement, no zombie generation, and terminal cleanup. | Reproduce rapid callback/close storms on hardware and prove Android/JmDNS callback behavior matches the deterministic state-machine contract. | **Medium, external evidence only.** Run the documented 50-toggle stress and retain generation/resource logs. |
...
| [#33 — Android outbound socket may use cellular](https://github.com/p2pKit/P2pKit/issues/33) | Commit `1ee7ee1` uses the selected `Network.socketFactory` for ordinary Wi-Fi/Ethernet and binds the explicit AP/tether local address before connect when Android exposes no `Network`. A manual peer resolves the same safe selector; a missing route or uncertain socket cleanup fails closed before another candidate. Android-host tests prove local binding, resolver precedence, typed failures, and retained cleanup. | Prove actual kernel route selection under Wi-Fi+cellular+VPN/hotspot on physical devices. | **High, external evidence only.** Run `LAN-T01`; close only with both-peer/PCAP route evidence. |
...
| [#35 — Android 800 ms rebind debounce](https://github.com/p2pKit/P2pKit/issues/35) | Serialized generations, rollback/retry, callback signals, the one-second AP-interface readiness watcher, and commit `1ee7ee1`'s atomic non-cancellable rebind transaction cover address arrival and superseding callbacks without lengthening the 800 ms debounce. | Measure rapid hotspot/Wi-Fi callback bursts across OEMs and determine whether the current readiness/state model covers every sequence. | **Medium.** Hardware measurement. Do not lengthen a timeout without evidence; run the 50-toggle case. |
...
| [#38 — iOS endpoint registry survives rebind](https://github.com/p2pKit/P2pKit/issues/38) | Commits `d6efe08` and `7af3a4b` invalidate opaque endpoints for stop, refresh, listener/browser terminal recovery, and every new generation. Browser retirement and the final registry clear now bracket queued callbacks atomically; old-path dials are fenced before the suspendable discovery hook, and final connected ownership is rechecked under the dial lock. | Prove the native path-change race on devices and confirm reconnect waits for a current browser result. | **High, external evidence only.** Apple path hardware. Execute `LAN-T07` B3/B4 with endpoint-generation and connection-ID evidence. |
...
| [#40 — asymmetric TCP connect timeouts](https://github.com/p2pKit/P2pKit/issues/40) | Platform timeouts are bounded and reconnect budgets are explicit. Commit `7af3a4b` also separates P2pKit's owned Apple deadline from a caller/parent timeout, preserving caller cancellation instead of misclassifying it as an internal connection failure. | JVM/Android remain 5 s and Apple 10 s; no hostile-network measurement justifies convergence or documents intentional asymmetry. | **Medium, external evidence only.** Two-machine/Apple timing. Measure first; change API/values only from evidence. |
| [#41 — iOS write-ready 10 s wedge](https://github.com/p2pKit/P2pKit/issues/41) | Commit `d6efe08` makes write-ready expiry terminal: it latches Closed, cancels the native connection exactly once, and makes the next write fail immediately. Commit `7af3a4b` preserves outer cancellation while keeping the internally owned write-ready/send deadlines terminal. Deterministic seams cover both classifications without changing the production ceiling. | Device evidence must measure cancellation/path-change wakeup and determine whether the 10-second ceiling is appropriate for AWDL. | **Medium, external evidence only.** Apple path fault injection. Run B3/B4 and the #40 timing campaign before changing the value. |
...
## Android selected-route and rebind transaction follow-up

Source commit `1ee7ee1cdca17879c9a5091e44aaf01a6131edc3` closes the
remaining repository-executable portions of Issues #28, #29, and #33 found by
the continuation audit:

- discovery and manual-peer dialing now use one shared route selector; it
  accepts an explicit Wi-Fi/Ethernet `Network`, rejects VPN/cellular overlays,
  and otherwise admits only a private address on a known AP/tether interface;
- one immutable route snapshot carries both the Android `Network` and selected
  local address from discovery into TCP dialing. Ordinary dials use that
  network's `socketFactory`; hotspot-host dials with no platform `Network`
  bind the selected AP address before connect;
- the last safe route survives a temporary JmDNS close/recreate window, while
  an initially unavailable route fails before an unrestricted socket or SYN is
  created. Manual peers resolve the same safe selector even when discovery is
  off;
- route setup and cleanup failures remain typed and owned. An uncertain close
  blocks later candidates until `stop()` retries it;
- superseding callback jobs may reset only the debounce. Once a JmDNS rebind
  begins, close/recreate/registration is non-cancellable under the lifecycle
  lock, so a newer callback or concurrent stop cannot strand a half-closed or
  zombie generation.

The implementation changes nine LAN production/test files. Deterministic
coverage adds address-order and VPN/cellular classification cases, missing-
route fail-closed behavior, AP local-address binding, manual-route resolution,
route-bind cleanup retry, superseded mid-close/mid-create transactions, and
stop during an active rebind.

Exact-tree local verification on 2026-08-12 passed:

- focused coordinator plus Android-host suites: 98 executions, zero failures,
  errors, or skips;
- `./gradlew :p2p-transport-lan:check :p2p-sample-android:check
  --rerun-tasks --dependency-verification=strict`: **PASS** with 123 JVM, 60
  Android-host, 89 Apple Simulator tests (88 executed; the single explicit
  60-second manual interoperability capture fixture remained ignored), and
  four Android sample tests; Android lint and ABI passed;
- `iosArm64`, `iosSimulatorArm64`, and `iosX64` production/test compilation and
  Simulator-arm64 linkage completed. The `iosX64Test` runtime task was skipped
  on the arm64 host after compilation/linkage, as designed;
- `git diff --check`: **PASS**.

No public ABI, Maven coordinate, LAN/secure-v2 wire format, platform floor, or
...
### Apple native ownership and cancellation follow-up

Source commit `7af3a4bb85d6a9b6f688bfe7245fc5f28028c889` (tree
`60e36765d6c650992c0bbf9e426411c4b6d67dd9`) closes the remaining
repository-executable Apple LAN ownership gaps found by the continuation
audit:

- each active listener has one native inbound owner. Queued callbacks from a
  retired listener are rejected and their unaccepted `nw_connection_t` is
  cancelled; wrapper construction cannot throw through the native callback;
  stop/rebind serializes admission with queue draining;
- an outbound native connection is cancelled if wrapper construction fails,
  and a connected dial transfers ownership only while its lifecycle generation
  remains current under the dial lock;
- listener/browser retirement fences pending dials, retires the browser result
  owner, and performs a final endpoint clear in that order, preventing a queued
  old-generation result from republishing a stale endpoint;
- discovery restoration reads current start/stop intent and current browser
  ownership, so stop during rebind cannot resurrect discovery and start during
  rebind cannot create a duplicate browser;
- delayed Bonjour reannounce work is cancelled and joined before advertising
  stops or its captured listener retires; and
- Apple connect, write-ready, and send-completion deadlines use owned timeout
  results. Caller cancellation and parent timeouts propagate unchanged, while
  genuine P2pKit deadline expiry retains the existing terminal cleanup and
  error behavior.

The six changed production/regression files are the Apple implementations and
tests for `IosLanDataTransport`, `IosLanDiscoveryTransport`, and
`IosRawConnection`. Deterministic tests cover stale/current inbound callbacks,
wrapper failure, admission-versus-stop, final dial handoff, browser
start/stop-versus-rebind, endpoint publication at the retirement boundary,
Bonjour nudge cancellation, and caller-owned timeout classification.

Final focused and affected-tree evidence on 2026-08-13:

- the focused cancellation/recovery run executed 28 tests with zero failures,
  errors, or skips;
- the complete arm64 iOS Simulator LAN suite registered 104 tests: 103
  executed successfully and only the explicit 60-second external
  `IosLanDiagnosticTest` capture fixture remained ignored;
- `iosArm64`, `iosSimulatorArm64`, and `iosX64` test binaries linked; ABI and
  Dokka passed. Runtime `iosX64Test` remains host-incompatible on Apple Silicon
  after successful compilation/linkage and is not inferred from that linkage;
- the local release gate passed root/module tests, Android compilation/lint and
  samples, CLI/Desktop checks, all 15 publication shapes, isolated JVM/Java/
  Android/KMP/iOS 14 consumers, the 38-component SBOM, release-XCFramework
  reconstruction/provenance, iOS 14 slice inspection, repository/link/policy
  checks, strict dependency verification, `git diff --check`, and the Swift
  sample build with warnings as errors.

No assertion or production timeout was weakened, and no public API/ABI,
coordinate, LAN or secure-v2 wire byte, platform floor, release tag, or
published artifact changes. Issues #23, #27, #38, #40, and #41 retain only the
device/AWDL/path or hostile-network measurements stated in the table above;
this host/simulator evidence does not upgrade those external campaigns.
