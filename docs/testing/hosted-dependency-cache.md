...
### Separate staging and empty-seed leaf, not its same-call parent
...
admission. Original indented canonical JSON remains exact bytes, not normalized
JSON. Supplied initializer directory identities, leaf-observed file stamps,
complete state rosters, source inputs and original properties are checked again
before return. These are point-in-time checks, not an atomic filesystem snapshot.

Stage exclusively creates an absent container with exactly `restore-home` (S)
and `staging.json`; even an existing empty container is rejected, never adopted
or deleted. Seed is read-only: it requires complete S emptiness and properties-
only H, with empty canonical evidence/cancellation directories. Every declared
artifact has an `ABSENT` inventory entry and all dependency counters are zero;
metadata I/O still occurs. This is not partial copying, provider qualification,
positive dependency population or resolver reuse.

Seed binds the supplied stage-return bytes and final RAW/LOCAL high-waters, not
just `staging.json`. Stage has hard120; seed hard120/new-work90. Both only shorten
against original proposed cumulative/job fences and independent LOCAL ceilings.
Serialization, callbacks, RAW-supplier return and leaf close spend the same caps;
equality is expired. The enclosing owner remains the caller's responsibility.
New factory returns are independently retained before fallible post-return checks;
only matching new owner rows may close. Preexisting parent resources are not
adopted, and unregistered returns remain retained UNKNOWN obligations. First
errors survive later cleanup failures; no failure owner or extra allowance is
created. Partial evidence and resource references remain on the original error.

All results retain `NOT_ADMITTED`, `NOT_PERFORMED` and false next-phase/export/
save authority. The separate bootstrap source roster does not widen ordinary
`files.INPUTS`, cache-key grammar, controllers, workflows or existing bootstrap
seed/export/save refusals. No dependency copy, producer, export/save, provider or
encrypted custody runs here. The [staging continuation](../maintenance/hosted-bootstrap-staging-2026-09-18.md)
separates executed offline controls, failed preimages and exact review scope.
...
seed/parent counterparts distinguish leaf ancestry from parent ancestry and bind
the final seed-parent RAW/LOCAL high-waters. Matching copied records remain data,
not authenticated original returns or single-use authority. Actual source/context/
properties/stage identities and bytes, properties-only H and empty S/evidence/
cancellation directories are rechecked using unchanged bounded staging readers.

The leaf exclusively creates `configuration-custody/request.json` and empty
`configuration-custody/retained` under the initializer session. Existing paths
refuse without adoption/deletion. One UUIDv4 binds the eventual product and same-home
stop to the canonical context job, not the initializer outer job. No canonical
`state/evidence/<id>` directory is allocated here, no Test loader installed, and no
ancestor chain or executable descriptor fabricated. Caller ID/path/command/key/
deadline overrides are absent. Only this module joins its separate custody roster;
ordinary source/key inputs, suppliers and selectors stay unchanged.

All work, readback, close and final checks share **custody-prepare at most120**,
shortened by original proposed cumulative/job and independent LOCAL fences. No
prepare-final/read allowance is borrowed or recorded as executed. First failures,
partial files and actual resource references remain; UNKNOWN grants no new owner.
...
and retains its closed return and final RAW/LOCAL high-waters. The separate parent
below now supplies NEW owner/handles under the original allocation in dormant
source; genuine composed/native qualification remains outstanding. It does not
adopt a returned prefix or revive old READ30.
...
The distinct NEW-owner source below supplies the next collection adapter, not
genuine source/native/service/recipient or file qualification. Honest no-loader
observation and export/freeze/save/probe/encrypted custody/seal/delivery remain
unfinished. Both HOLDs, exact selectors, credentials,
...
### Original-call NEW-owner collection, not no-loader/cache acceptance

Internal `collect_after_entry(transition)` claims before the original binding
call and retains its actual return before fallible checks. BOUND/COMPLETE
predecessors stay closed; no returned prefix, constructor or repeated attempt
creates execution authority. A distinct owner/reader/window uses WORK120,
FINAL≤actual-start45/first165 and READ≤actual-start30/first195, intersecting the
original cumulative/job and LOCAL caps. READ is post-close return accounting
only, **not file-read execution**; all new admission/file/copy work is in WORK.

New handles compare producer start/receipt bytes and original bindings; the
first manifest read is explicitly4MiB. Two extra collection-source file bindings
surround NEW native readmission without widening shared stage/cache/source
rosters. The actual once-only leaf return is retained before validation. Empty
retained/ before copy and complete current membership after copy are distinct
oracles. Warmed H is not an attested cohort. Descendant handles verify their own
...
survive terminal refusal. Only source-selected successful session-writer/readback
additions are allowed. Pre-final uncertainty quarantines without a finalizer;
actual returned-finalizer facts remain separate from failed terminal validation.
This cannot prove no query row disappeared before the pre-final snapshot.

The [collection-parent continuation](../maintenance/hosted-bootstrap-collection-parent-2026-09-19.md)
records R1's two independent findings, failed setup and12-failure/2-pass aggregate,
and R2's78 distinct author AST/memory passes in five disjoint invocations.
Separate independent25 close/query memory controls passed in one bounded
invocation; source-only approval and the original-result clarification are
separately bound. Eight source-only gate results are recorded there; the
containing commit's issue mapping supplies the distinct final documentation
checks and complete-patch review, not implied by these control counts.
The new historical/host boundaries, inventory/copy operation, upstream/native
suppliers and filesystem are explicit models; no real native/file/hosted or
provider qualification follows. No-loader remains NOT_OBSERVED; population,
test, admitted-budget and next-phase/export/save authority remain nonaccepting.
...
The fixed command below now invokes this leaf, but no workflow invokes that
command. Ordinary save-set/bootstrap guards remain untouched.
...
### Fixed post-save guard, not provider enforcement

Dormant `after-save` in
[`run-hosted-cache-bootstrap.py`](../../scripts/run-hosted-cache-bootstrap.py)
requires the original producer, save-preparation and save outcomes to be literal
`success`, plus the three original command-exported hashes. The new environment
bindings are `P2PKIT_BOOTSTRAP_SAVE_PREPARE_OUTCOME`,
`P2PKIT_BOOTSTRAP_SAVE_PREPARATION_SHA256` and `P2PKIT_BOOTSTRAP_SAVE_OUTCOME`;
the producer bindings are unchanged. Only future fixed trusted workflow glue
can supply genuine original outcomes and hashes. Declarations, conclusions and
self-computed file hashes cannot authenticate themselves.

The earliest command-entry RAW/LOCAL pair is a **post-action upper bound**, not
an actual provider start or return. It must precede the unchanged original
provider end. Later validation spends a distinct original `save-readmission`120
cap; it does not renew180 or require that later work finish inside the already
observed provider interval. Fixed descriptor/package paths, original directory/
file bindings, fresh native admission, original allocation and current plan inputs
are rechecked. The existing `Originals`/`StagedEvidence` constructors validate
the exact retained context, canonical, properties and stage/seed bytes; no old
live owner, capability or private execution graph is reconstructed.

One NEW file-only owner invokes the existing complete `after_save` traversal
once under120/90. Its after-only reentry option retains historical before-parent
LOCAL solely as old-process data; a distinct current-process readmission-close
RAW/LOCAL floor constrains this new phase. Same-process defaults remain. Known
parent closure requires the original complete owned roster, no retained failure
or UNKNOWN, and a timely actual post-close observation—not merely `closed=True`.
The next save-observation30 writer retains that previous close and the existing
save classification (**STORAGE_UNPROVEN**); a separate save-owner-return45 writer
retains the observation owner's actual close. Its own return remains PENDING.
Only the final guarded digest escapes publicly, provisional until actual command
and future original step success. This is success-only composition, not guaranteed
encrypted delivery of failed/unknown provider or leaf evidence.

This guard **does not stop a provider at its absolute end or establish provider
...
26 command, six descriptor and eight mapping/window controls. Selected actual
Owner/admission composition, supplied-input/canonical validators, mapper, bound
readers, after-save traversal and private writers use memory files. Native admission,
source allowlist acquisition, allocation and elapsed clocks remain models; the
isolated timing case also models mapper/leaf returns. No full runner import,
native/private-file/provider, Gradle or hosted execution occurred. The first smoke
remains0pass/3errors (fixture treated a named tuple as a dataclass); corrected3
passed. The first35 aggregate remains34pass/1failure (wrong expected refusal code),
not an intentional preimage regression. Fixture corrections changed no implementation
bytes. Final40 is separate, not an added total. The bounded run retained
timeout20/kill2, CPU10/AS512MiB/FD64/core0 and1MiB regular outputs; bindings stayed
unchanged, own process group was absent and guard denials0. UID0/audit hooks are
...
handoff/entry/service-time/allocation-proposal/close/history/legacy-reader/fresh-entry/recipient-child/recipient-parent/initialization/staging-leaf/staging-parent/configuration-custody-reservation/configuration-custody-parent/configuration-producer-parent/configuration-report-inventory/configuration-file-copy/producer-to-collection-binding/original-call-collection-parent
...
The collection-parent example selects21 of78 new methods. Its five disjoint
author invocations and one separate25-control independent invocation are
