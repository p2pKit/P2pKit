...
- **`:p2p-sample-desktop`** (JVM CLI), **`:p2p-sample-desktop-ui`** (Compose Desktop), **`:p2p-sample-android`** (Compose), **`:sample-kmp-shared`**, **`:iosApp`** (Xcode project generated by xcodegen from `samples/iosApp/project.yml`; pre-build script runs Gradle via `sh ./gradlew`) — test harnesses, not published.
