...
def initial_collect_authority_command(context_hash, minimum=None):
    """Only the fresh-token post-export HTTP child, not authority-1 or crypto."""
    result = initial_command(context_hash, minimum)
    result[4] = str(SCRIPTS / "run-hosted-initial-recipient-custody.py")
    result[5] = "_post-export-authority"
    return result
...
def phase(owner, private, context_raw, token, fence, *, initial_git=None):
    ...
    try:
        ...
        if collect_phase is not None:
            owner.error("collect-phase-setup", error)
            try:
    ...
    try:
        ...
        elif collect_phase is not None:
            try:
                owner.leave_collect_phase(*collect_phase)
            except BaseException as error:
