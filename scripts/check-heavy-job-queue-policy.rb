...
module HeavyJobQueuePolicy
    ...
    JOBS = {
        ...
        "desktop-cross-host.yml" => {"verify" => "${{ matrix.os }}", "windows-directory-fsync-control" => "windows-directory-fsync-control",
            "mac-host-admission-probe" => "mac-host-admission-probe", "dependency-lock-candidate" => "dependency-lock-candidate"},
    ...
    WORKFLOW_CONCURRENCY = {
        ...
        "desktop-cross-host.yml" => {
            "group" => "desktop-cross-host-${{ github.event_name == 'workflow_dispatch' && (inputs.operation == 'dependency-lock-candidate' || inputs.operation == 'dependency-lock-candidate-x64' || inputs.operation == 'dependency-lock-candidate-macos14') && format('lock-{0}-{1}', github.run_id, github.run_attempt) || github.event_name == 'workflow_dispatch' && (inputs.operation == 'windows-directory-fsync-control' || inputs.operation == 'macos-arm64-admission' || inputs.operation == 'macos-x64-admission') && format('control-{0}-{1}', github.run_id, github.run_attempt) || github.ref }}",
            "cancel-in-progress" => "${{ github.event_name != 'workflow_dispatch' || (inputs.operation != 'windows-directory-fsync-control' && inputs.operation != 'macos-arm64-admission' && inputs.operation != 'macos-x64-admission' && inputs.operation != 'dependency-lock-candidate' && inputs.operation != 'dependency-lock-candidate-x64' && inputs.operation != 'dependency-lock-candidate-macos14') }}",
        },
    ...
    CONDITIONS = {
        ...
        ["desktop-cross-host.yml", "verify"] => "${{ github.event_name != 'workflow_dispatch' || (inputs.operation != 'windows-directory-fsync-control' && inputs.operation != 'macos-arm64-admission' && inputs.operation != 'macos-x64-admission' && inputs.operation != 'dependency-lock-candidate' && inputs.operation != 'dependency-lock-candidate-x64' && inputs.operation != 'dependency-lock-candidate-macos14') }}",
...
if $PROGRAM_NAME == __FILE__
    ...
    puts "RESULT: PASS — eight participating jobs share the bounded non-cancelling queue; workflow groups remain separate"
end
