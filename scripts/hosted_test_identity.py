...
def _admit(profile, env, event_raw, git, now):
    ...
    if event_name == "pull_request":
        ...
        if profile == "desktop":
            require(type(inputs) is dict and set(inputs) == {"operation", "expected_sha", "expected_tree",
                    "reviewed_base", "evidence_public_key", "evidence_fingerprint"} and
                    inputs["operation"] == "desktop" and all(value == "" for key, value in inputs.items()
                                                              if key != "operation"), "IDENTITY_MANUAL_INPUTS")
        else:
