...
FROZEN_SPEC = importlib.util.spec_from_file_location("frozen_set_fixture_policy", ROOT / "scripts/release_application_set.py")
F = importlib.util.module_from_spec(FROZEN_SPEC)
FROZEN_SPEC.loader.exec_module(F)
VERSION = "0.8.0-rc1"
...
class FakeApi:
    ...
    def json(self, path, *, method="GET", value=None, missing=False, upload=None):
        ...
        if path.startswith("/actions/runs/900/attempts/") and path not in self.data:
            return copy.deepcopy(self.data["/actions/runs/900"])
        return copy.deepcopy(self.data[path])
...
class ReleaseTest(unittest.TestCase):
    ...
    def bundle(self, platform, arch, extra=None, context_change=None):
        ...
        context = {"repository": R.REPO, "commit": SOURCE, "tree": TREE, "event_name": "push", "ref": "refs/heads/main",
            ...
            "workflow_sha": SOURCE, "platform": "linux" if platform == "android" else platform, "architecture": arch, "canonicalVersion": VERSION}
        context.update(context_change or {})
        binding = R.VERSION.version_fields(VERSION)
        embedded = R.VERSION.embedded_identity(VERSION, SOURCE)
        manifest = {"schema": 3, "sourceAndRun": context, "scope": R.PACK.SCOPE, "versionBinding": binding}
        if platform == "android":
            manifest.update(artifact=artifacts[0], agpMetadata={"applicationId": "dev.p2pkit.sample.android", "variant": "debug",
                "versionCode": binding["androidVersionCode"], "versionName": VERSION},
                apkInspection={"binaryManifest": {"applicationId": "dev.p2pkit.sample.android", "versionCode": binding["androidVersionCode"],
                "versionName": VERSION}, "embeddedIdentity": embedded, "signerIdentity": "NOT_VERIFIED"})
        else:
            fields = {"linux": {"Package": "p2pkit-sample", "Version": binding["debianVersion"], "Architecture": "amd64"},
                "windows": {"ProductName": "P2pKit Sample", "ProductVersion": binding["nativeVersion"], "Template": "x64;1033"},
                "macos": {"CFBundleShortVersionString": binding["nativeVersion"], "CFBundleVersion": binding["nativeVersion"]}}[platform]
            artifacts[-1]["nativeMetadata"] = {"fields": fields, "embeddedIdentity": embedded, "reader": "SYNTHETIC_NOT_NATIVE_EVIDENCE"}
            manifest["artifacts"] = artifacts
            manifest["layoutInspection"] = {"embeddedIdentity": embedded}
    ...
    def maven_fixture(self):
        ...
        self.api.collections["/actions/runs/900/attempts/1/jobs"] = [
            {"id": 910 + i, "name": name, "run_id": 900, "run_attempt": 1, "head_sha": SOURCE,
             "status": "completed", "conclusion": "success", "started_at": "2026-09-20T14:59:00Z", "completed_at": "2026-09-20T15:00:00Z"}
            for i, name in enumerate(("freeze-applications", "verify-release", "publish-release"))]
        self.api.data["/environments/maven-central"] = {**copy.deepcopy(self.api.data["/environments/" + R.ENVIRONMENT]),
            "id": 601, "name": "maven-central"}
        self.api.collections["/environments/maven-central/deployment-branch-policies"] = [{"name": "v*", "type": "tag"}]
    ...
    def prepared(self, plan):
        """Construct original in-memory tag-run evidence; never hosted evidence.

        The actual frozen reader, live metadata checks and both approval checks
        execute against FakeApi. No publication gate is mocked to success.
        """
        if "frozenApplicationSet" in plan:
            return plan
        publication = R.maven_publication(self.api, SOURCE, 900, 1)
        qualification = {k: v for k, v in plan.items() if k != "mavenPublication"}
        with tempfile.TemporaryDirectory(dir=self.base) as temporary:
            packages = F.inspect_originals(self.api, qualification, Path(temporary))
        document = {"schema": 1, "scope": F.SCOPE, "maven": F.invocation_fields(publication),
            "qualification": qualification, "environment": F.protected_environment(self.api),
            "preparedAt": int(NOW) - 1, "expiresAt": F.expiry(qualification), "packages": packages}
        raw, stream = R.encoded(document), io.BytesIO()
        with zipfile.ZipFile(stream, "w") as archive:
            archive.writestr(zipfile.ZipInfo(F.FILE), raw)
        binary = stream.getvalue()
        artifact = {"id": 9001, "name": F.set_name(publication), "expired": False,
            "created_at": "2026-09-20T15:00:00Z", "expires_at": "2026-10-04T15:00:00Z",
            "size_in_bytes": len(binary), "digest": "sha256:" + hashlib.sha256(binary).hexdigest(),
            "workflow_run": {"id": 900, "head_sha": SOURCE, "head_branch": publication["tag"]}}
        self.api.data["/actions/artifacts/9001"] = artifact
        self.api.files[9001] = binary
        self.api.collections["/actions/runs/900/artifacts"] = [artifact]
        self.api.data["/actions/runs/900/approvals"] = [{"state": "approved", "user": OWNER,
            "comment": F.approval_line(document), "environments": [{"id": 601, "name": "maven-central"}]}]
        plan.update(R.frozen_publication_plan(self.api, publication))
        return plan
    ...
    def test_complete_metadata_admits_exact_four_outputs_without_mutation(self):
        ...
        self.assertEqual("samples-v" + VERSION, plan["tag"])
    ...
    def test_trigger_event_set_is_explicit_for_each_maintained_workflow(self):
        for name, path in R.TRIGGERS.items():
            ...
            for event in ("push", "schedule", "workflow_dispatch", "pull_request", "pull_request_target"):
                ...
                with self.subTest(name=name, event=event), self.assertRaises(R.Hold):
    ...
    def test_public_apps_exclude_review_and_private_evidence_and_all_get_anonymous_probes(self):
        ...
        self.assertTrue(result["publicFullDownloadHashesVerified"])
    ...
    def test_anonymous_probe_uses_no_credentials_even_on_redirect(self):
        ...
        class Opener:
            def open(self, request, timeout):
                ...
                return Response(b"synthetic payload")
        self_outer = self
        api = R.Api("synthetic-fixture-token-not-a-credential")
        api.opener = Opener()
        api.public_probe("samples-" + SOURCE, {"file": "example.apk", "bytes": 17, "sha256": hashlib.sha256(b"synthetic payload").hexdigest()})
        self.assertEqual(2, len(calls))
        self.assertTrue(all(x.get_header("Authorization") is None and x.get_header("Range") is None for x in calls))

    def test_anonymous_full_download_rejects_a_wrong_last_byte_or_partial_body(self):
        class Response(io.BytesIO):
            status = 200
            headers = {"Content-Length": "17"}
        asset = {"file": "example.apk", "bytes": 17, "sha256": hashlib.sha256(b"synthetic payload").hexdigest()}
        api = R.Api("synthetic-fixture-token-not-a-credential")
        api.opener = mock.Mock()
        for raw in (b"s", b"synthetic payloaX", b"synthetic payloadX"):
            api.opener.open.return_value = Response(raw)
            with self.subTest(raw=raw), self.assertRaises(R.Hold):
                api.public_probe("samples-v" + VERSION, asset)
        partial = Response(b"synthetic payload")
        partial.status = 206
        partial.headers = {"Content-Range": "bytes 0-16/17"}
        api.opener.open.return_value = partial
        with self.assertRaises(R.Hold):
            api.public_probe("samples-v" + VERSION, asset)

    def test_schema_two_preview_is_not_qualified_for_versioned_delivery(self):
        plan = self.plan()
        artifact = plan["artifacts"][0]
        with zipfile.ZipFile(io.BytesIO(self.api.files[artifact["id"]])) as archive:
            manifest = json.loads(archive.read("manifest.json"))
        manifest["schema"] = 2
        raw = self.bundle("android", "x64", extra={"manifest.json": R.encoded(manifest)})
        path = self.base / "historical-preview.zip"
        path.write_bytes(raw)
        artifact.update(size_in_bytes=len(raw), digest="sha256:" + hashlib.sha256(raw).hexdigest())
        with self.assertRaises(R.Hold):
            R.inspect_bundle(path, artifact, plan)
    ...
    def test_maven_completion_binds_version_and_reuses_exact_qualified_main_apps(self):
        ...
        self.assertEqual({"freeze-applications", "verify-release", "publish-release"}, {x["name"] for x in plan["mavenPublication"]["jobs"]})
    ...
    def test_maven_completion_is_rebound_in_admit_prepare_and_publish_entrypoints(self):
        ...
        captured = []
        def stop_at_admit(api, publication, binding):
            captured.append((publication["source"], publication["id"], publication["attempt"], binding))
        ...
        for operation, job in (("admit", "admit"), ("prepare-review", "prepare-review"), ("publish", "publish")):
            with mock.patch.dict(os.environ, {"GITHUB_EVENT_NAME": "workflow_run", "GITHUB_EVENT_PATH": str(event_path),
                    ...
                    mock.patch.object(R, "Api", return_value=self.api), mock.patch.object(R, "frozen_publication_plan", side_effect=stop_at_admit), \
                    mock.patch("sys.argv", ["publish-sample-release.py", operation, "--source", SOURCE,
                                           "--maven-run", "900", "--maven-attempt", "1"]), \
                    mock.patch("sys.stdout", new=io.StringIO()):
                self.assertEqual(1, R.main())
        self.assertEqual([(SOURCE, 900, 1, None)] * 3, captured)
        self.assertEqual([], self.api.mutations)
