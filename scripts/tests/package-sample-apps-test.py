...
SHA, TREE = "a" * 40, "b" * 40
VERSION = "0.8.0-rc1"
FIXTURE_SPEC = importlib.util.spec_from_file_location("sample_binary_manifest", Path(__file__).parent / "fixtures/sample_binary_manifest.py")
...
class PackageFixtures(unittest.TestCase):
    def setUp(self):
        ...
        self.root.mkdir()
        (self.root / "gradle.properties").write_text("VERSION_NAME=" + VERSION + "\n")
        native = mock.patch.object(subject.IDENTITY, "inspect_native", return_value={"reader": "SYNTHETIC_NOT_NATIVE_ACCEPTANCE"})
        native.start()
        ...
        self.identity = {"repository": subject.REPOSITORY, "commit": SHA, "tree": TREE, "platform": "linux",
                         "architecture": "x64", "canonicalVersion": VERSION, "event_name": "push", "ref": "refs/heads/main",
    ...
    def jar(self, path, main):
        ...
        with zipfile.ZipFile(path, "w") as archive:
            ...
            if main == "dev/p2pkit/sample/desktop/ui/MainKt.class":
                archive.writestr("p2pkit-release.json", json.dumps(subject.IDENTITY.VERSION.embedded_identity(VERSION, SHA)))
    ...
    def android_fixture(self):
        ...
        with zipfile.ZipFile(apk, "w") as archive:
            archive.writestr("AndroidManifest.xml", fixture.manifest())
            archive.writestr("assets/p2pkit-release.json", json.dumps(subject.IDENTITY.VERSION.embedded_identity(VERSION, SHA)))
        ...
        metadata = {"version": 3, "artifactType": {"type": "APK", "kind": "Directory"},
                    "applicationId": "dev.p2pkit.sample.android", "variantName": "debug", "elements": [
                        {"type": "SINGLE", "filters": [], "versionCode": 320202, "versionName": VERSION, "outputFile": apk.name}]}
    ...
    def test_android_metadata_is_actual_single_debug_output_not_path_guesses(self):
        ...
        destination.mkdir()
        result = subject.android_artifact(self.root, destination, self.identity)
        self.assertEqual("sample-debug.apk", result["artifact"]["file"])
        self.assertEqual("debug", result["agpMetadata"]["variant"])
        self.assertEqual("NOT_VERIFIED", result["apkInspection"]["signerIdentity"])
        self.assertEqual(320202, result["apkInspection"]["binaryManifest"]["versionCode"])
        self.assertEqual(SHA, result["apkInspection"]["embeddedIdentity"]["sourceCommit"])
        ...
        for key, value in mutations:
            ...
            with self.subTest(key=key, value=value), self.assertRaises(ValueError):
                subject.android_artifact(self.root, destination, self.identity)
