...
class IdentityTests(unittest.TestCase):
    ...
    def test_build_only_and_special_manual_operations_are_not_test_custody(self):
        self.manual()
        for operation in ("desktop", "sample-apps", "dependency-lock-candidate", "dependency-lock-candidate-x64",
                          "windows-directory-fsync-control", "windows-helper-controls", "macos-arm64-admission", "unknown"):
            with self.subTest(operation=operation):
                self.event["inputs"]["operation"] = operation
                self.rejected("IDENTITY_MANUAL_INPUTS")

    def test_input_free_manual_forms_read_only_actual_main_policy(self):
        self.manual()
        for shape in (None, {}):
            with self.subTest(shape=shape):
                self.event["inputs"] = shape
                self.git.requested.clear()
                admitted = self.admit()
                self.assertEqual(self.git.requested, [BASE])
                self.assertFalse(H.sample_packaging_required(admitted))
        self.event.pop("inputs")
        self.git.requested.clear()
        self.assertFalse(H.sample_packaging_required(self.admit()))
        self.assertEqual(self.git.requested, [BASE])

    def test_empty_legacy_campaign_fields_are_not_an_input_free_dispatch(self):
        self.manual()
        self.event["inputs"] = {"operation": "desktop", "expected_sha": "", "expected_tree": "", "reviewed_base": "",
                                "evidence_public_key": "", "evidence_fingerprint": ""}
        self.rejected("IDENTITY_MANUAL_INPUTS")
        self.assertEqual(self.git.requested, [])

    def test_any_manual_override_refuses_before_policy_acquisition(self):
        self.manual()
        for field in ("operation", "expected_sha", "expected_tree", "reviewed_base", "evidence_public_key",
                      "evidence_fingerprint", "recipient", "release", "profile"):
            for value in ("", "synthetic-override", None, False):
                with self.subTest(field=field, value=value):
                    self.event["inputs"] = {field: value}
                    self.rejected("IDENTITY_MANUAL_INPUTS")
                    self.assertEqual(self.git.requested, [])
        for value in (False, 0, "", [], ["desktop"]):
