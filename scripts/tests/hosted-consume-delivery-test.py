...
class ConnectedBase(M.Base):
    ...
    def sample_fixture(self, role=None):
        ...
        current = {"repository": C.identity.REPOSITORY, **original["source"], "run_id": github["runId"],
            ...
            "architecture": arch, "osRelease": "SYNTHETIC", "python": "SYNTHETIC", "translation": "SYNTHETIC"}
        ...
        for name in ("desktop-ui-" + label + suffix, "desktop-cli-" + label + suffix,
            ...
            artifacts.append({"file": name, **P.file_hash(desktop / name)})
        P.finish_manifest(self.root, desktop, current, {"artifacts": artifacts})
        if platform == "linux":
