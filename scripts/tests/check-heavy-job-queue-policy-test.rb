...
%w[sample-apps windows-helper-controls dependency-lock-candidate iphoneos-product].each do |id|
    mutations["unreviewed Desktop campaign #{id}"] = ["participating job IDs", ->(w) {
        w["desktop-cross-host.yml"]["jobs"][id] = {"concurrency" => copy(POLICY::QUEUE)}
    }]
end
...
mutations.each do |name, (diagnostic, mutate)|
    ...
    mutate.call(changed)
    raise "queue mutation had no effect: #{name}" if Marshal.dump(changed) == original
