...
PATH="$WORK_DIR/bin:$PATH" FAKE_CENTRAL_STATE_DIR="$WORK_DIR/state" \
    FAKE_MAVEN_CODE=404 "$SCRIPT" absent >/dev/null

if PATH="$WORK_DIR/bin:$PATH" FAKE_CENTRAL_STATE_DIR="$WORK_DIR/state" \
    FAKE_MAVEN_CODE=404 FAKE_MAVEN_EXISTING_ARTIFACT=p2p-core \
...
if PATH="$WORK_DIR/bin:$PATH" FAKE_CENTRAL_STATE_DIR="$WORK_DIR/state" \
    FAKE_MAVEN_CODE=500 "$SCRIPT" absent >/dev/null 2>&1; then
