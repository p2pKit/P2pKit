...
def android_artifact(root, destination):
    ...
    require("/" not in name and name.endswith(".apk") and item.get("type") == "SINGLE" and item.get("filters") == [] and
            type(item.get("versionCode")) is int and item["versionCode"] == 1 and item.get("versionName") == "0.1.0",
    ...
    return {"artifact": artifact, "agpMetadata": {"applicationId": metadata["applicationId"], "variant": "debug",
            ...
            "apkInspection": "ZIP structure/CRC only; binary manifest and signer identity not verified",
...
def package(root, output):
    ...
    try:
        ...
        if identity["platform"] == "linux":
            ...
            android.mkdir()
            finish_manifest(root, android, identity, android_artifact(root, android))
